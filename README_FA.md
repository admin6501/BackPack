...
برای تعیین سقف حجم یک تونل، از `Manage → Manage Tunnels → نام تونل → Traffic quota`
یا صفحهٔ **Edit** همان تونل در پنل وب استفاده کن. عدد را به GiB و بدون اعشار
وارد کن؛ `0` یعنی نامحدود. `Manage → Tunnel Metrics` و صفحهٔ **Metrics** پنل،
ورودی، خروجی، مصرف کل و حجم باقی‌مانده را جدا نشان می‌دهند. وقتی حجم تمام شود،
تونل و پورت‌های فورواردش موقتاً بسته می‌شوند. با افزایش سقف یا صفرکردن آن دوباره
راه می‌افتد و مصرف قبلی پاک نمی‌شود. اگر می‌خواهی حجم کاربران را محدود کنی، سقف
را روی **سرور ورودی ایران** بگذار؛ هر سرور شمارندهٔ خودش را دارد و نباید مصرف
دو سر تونل را با هم جمع کنی. [جزئیات محدودیت حجم](docs/limits.md).
...
Protocol v2 برای آی‌پی واقعی کاربر؛ سقف اتصال، پهنای باند و حجم هر تونل؛ داشبورد
